...
- Мультиколлинеарность не нарушает предпосылок теоремы Гаусса—Маркова: оценки остаются несмещёнными, просто их дисперсия (а значит, и стандартные ошибки) слишком велика.
...
**Мультиколлинеарность** (multicollinearity) — наличие линейной зависимости между объясняющими переменными модели. Формально различают два вида:

| Вид | Что означает |
|---|---|
| Полная | Функциональная (тождественная) линейная зависимость между регрессорами |
| Частичная (или просто мультиколлинеарность) | Более или менее сильная корреляция между факторами |

В терминологии лектора связь **двух** регрессоров называется **коллинеарностью**, трёх и более — мультиколлинеарностью.

Ключевая характеристика явления: все предпосылки теоремы Гаусса—Маркова при мультиколлинеарности **выполнены**, оценки остаются несмещёнными — но их дисперсия слишком высока, отсюда большие стандартные ошибки и низкая значимость коэффициентов.
...
> [!NOTE]
> Мультиколлинеарность **не обязательно** приводит к плохим оценкам. Если наблюдений много, выборочные дисперсии регрессоров велики, а дисперсия случайного члена мала — результат может оказаться вполне приличным. Устранять её начинают только тогда, когда она серьёзно влияет на оценку регрессии.
...
Тогда $r = 1$ (при $k > 0$) или $r = -1$ (при $k < 0$). Оценка первого коэффициента в модели с двумя регрессорами вычисляется по формуле

$$b_1 = \frac{\mathrm{Cov}(x_1, y)\,\mathrm{Var}(x_2) - \mathrm{Cov}(x_2, y)\,\mathrm{Cov}(x_1, x_2)}{\mathrm{Var}(x_1)\,\mathrm{Var}(x_2) - \mathrm{Cov}^2(x_1, x_2)}$$

Подставив $x_2 = k x_1$, получаем в числителе два одинаковых слагаемых, вычитающихся друг из друга, и то же самое в знаменателе:

$$b_1 = \frac{k^2\,\mathrm{Cov}(x_1, y)\,\mathrm{Var}(x_1) - k^2\,\mathrm{Cov}(x_1, y)\,\mathrm{Var}(x_1)}{k^2\,\mathrm{Var}^2(x_1) - k^2\,\mathrm{Var}^2(x_1)} = \frac{0}{0}$$
...
Матрица парных коэффициентов корреляции между регрессорами, симметричная относительно главной диагонали:

$$R = \begin{pmatrix} 1 & r_{x_2 x_1} & \dots & r_{x_k x_1} \\ r_{x_1 x_2} & 1 & \dots & r_{x_k x_2} \\ \dots & \dots & \dots & \dots \\ r_{x_1 x_k} & r_{x_2 x_k} & \dots & 1 \end{pmatrix}$$

Порог: $r_{x_i x_j} > 0{,}7$ — переменные явно коллинеарны.
...
> [!NOTE]
> Никакой дополнительной информации по сравнению с $R_i^2$ показатель VIF не несёт — судить можно и напрямую по $R_i^2$. Это вопрос сложившейся традиции, а не математики. Порог по $R_i^2$ при этом принято брать мягче: значение выше $0{,}6$ уже считается признаком мультиколлинеарности, тогда как $\mathrm{VIF} > 10$ соответствует $R_i^2 > 0{,}9$.

> [!IMPORTANT]
> Нужно уметь считать VIF руками, зная $R_j^2$ вспомогательных регрессий, а не только вызывать `vif()` из пакета `car`.
...
$X$ — матрица объясняющих переменных размера $n \times k$, к которой слева дописан **столбец единиц** (он нужен, чтобы оценить свободный член $a$):

$$X = \begin{pmatrix} 1 & x_{12} & \dots & x_{1k} \\ \dots & \dots & \dots & \dots \\ 1 & x_{n2} & \dots & x_{nk} \end{pmatrix}$$

Далее строится матрица $X'X$ ($X'$ — транспонированная $X$) и вычисляются её собственные значения $\lambda$:
...
Граница в 30 не строгая: по сути мультиколлинеарность — это субъективное недовольство исследователя слишком высокими стандартными ошибками коэффициентов.
...
На паре разбирался пример с квартирами; в презентации «Мультиколлинеарность. Часть 1» тот же материал показан на встроенном датасете `cars` (данные 1920-х: скорость в милях в час, тормозной путь в футах).

**Полиномиальная регрессия тормозного пути** — эталонный пример мультиколлинеарности: степени одной и той же переменной коррелируют почти идеально.

```r
m.poly3 <- lm(dist ~ speed + I(speed^2) + I(speed^3), data = cars)
```

`I(...)` заставляет R понимать выражение в скобках в обычном математическом смысле, а не в «регрессионном»: `y ~ x + z` — регрессия на две переменные, `y ~ I(x + z)` — на одну переменную $x+z$; `y ~ (x + z + w)^2` — на $x, z, w$ и все их попарные произведения.

Результаты примера:

| Показатель | Значение |
|---|---|
| F-статистика | $31{,}58$, p-value $= 3{,}07 \cdot 10^{-11}$ — регрессия значима |
| p-value коэффициентов | $0{,}496$; $0{,}323$; $0{,}488$; $0{,}369$ — не значим ни один |
| Корреляции регрессоров | `speed`—`speed²` $= 0{,}98$, `speed²`—`speed³` $= 0{,}99$, `speed`—`speed³` $= 0{,}94$ |
| CI | $87\,480$ при пороге 30 |
| VIF | $274$, $1408$, $483$ при пороге 10 |

**Центрирование переменных — ловушка.** Если вычесть из регрессора среднее (`cars$spc <- cars$speed - mean(cars$speed)`) и переоценить модель, коэффициенты становятся значимыми. Это иллюзия: у оценок просто другая ковариационная матрица, а прогноз и его доверительный интервал не меняются ни на йоту.

**Два типа интервалов при прогнозе** (опция функции `predict`):

| Опция | Что оценивает |
|---|---|
| `'confidence'` | Доверительный интервал для математического ожидания будущего $y$ |
| `'prediction'` | Прогнозный интервал для индивидуального значения $y$ |

Опция `se.fit = TRUE` возвращает стандартную ошибку $\hat{y}$, функция `vcov(model)` — ковариационную матрицу оценок коэффициентов.

**Способы борьбы с мультиколлинеарностью** (слайды 24–26) разбирались на следующей паре — см. [лекцию 4](04-fiktivnye-peremennye-i-otbor-regressorov.md). Помимо перечисленных там, в слайдах названы два «нулевых» варианта: смириться с высокими стандартными ошибками и увеличить количество наблюдений. Логика остальных способов такая: обычная регрессия ищет несмещённые оценки с наименьшей дисперсией, а можно сознательно перейти к **смещённым** оценкам — любая смещённая оценка $\beta$ есть несмещённая оценка какого-то другого коэффициента $\gamma$.
...
| Полная мультиколлинеарность | Функциональная (тождественная) линейная связь регрессоров; $r = \pm 1$, коэффициенты не вычисляются, программа даёт сингулярность |
| Частичная мультиколлинеарность | Более или менее сильная корреляция регрессоров — обычный рабочий случай |
